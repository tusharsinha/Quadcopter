...
// Checking Activity on each axes
...
// Checking Inactivity on each axes
...
//Checking AC/DC coupled Activity or inactivity 0 - DC and 1 - AC
...
//Setting AC/DC coupled Activity or inactivity 0 - DC and 1 - AC
...
//Checking Suppression bit
bool ADXL345::getSuppressBit(){
  return getRegisterBit(ADXL345_TAP_AXES, 3);
}

//Writing Suppression bit
void ADXL345::setSuppressBit(bool state) {  
  setRegisterBit(ADXL345_TAP_AXES, 3, state);
}


//Checking Tap detection on X,Y,Z and writing Tap detection
...
void ADXL345::setRate(float rate){
  ...
  int v = (int) (rate / 6.25);
  int r = 0;
  while (v >>= 1)  // Shifts bits to left and check nullity thus r returns maximum bit
  {
    r++;
  }
  if (r <= 9) {
...
//Sets whole BW byte (set within 3<x<1600) 0000xxxx for normal 0001xxxx for low power though
//following code will print error in case of low power so use previous one for use in low power
...
//Read data rate and power mode register
...
//Read source of Interrupt from all possible interrupts
...
//Read on bit (particular interrupt)
bool ADXL345::getInterruptSource(byte interruptBit) {
  return getRegisterBit(ADXL345_INT_SOURCE,interruptBit);
}

//Read mapping of particular bit
